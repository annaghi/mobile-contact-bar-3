(function ($, document) {
    ...
    var model = {
        ...
        onReady: function () {
            ...
            $('#mcb-model-mobile-draggable')
                .mousedown(function (event) {
                    ...
                    var mgt = $('#mcb-model-mobile-group')
                            .attr('transform')
                            .match(/\w+\(([^,)]+),([^)]+)\)/),
                        vp = $('#mcb-bar-vertical_alignment input:checked').val(),
            ...
            $('#mcb-bar-horizontal_alignment input').on('change', model.update_bar_group_x);

            $('#mcb-bar-vertical_alignment input').on('change', function () {
                var mgt = $('#mcb-model-mobile-group')
                    .attr('transform')
                    .match(/\w+\(([^,)]+),([^)]+)\)/);
            ...
            // Items
            $('.mcb-contact-checkbox').on('change', model.update_bar_items);
            // $('#mcb-icons_labels-icon_color input').on('change', model.update_bar_items);
        },

        update_bar_group_x: function () {
            var hp = $('#mcb-bar-horizontal_alignment input:checked').val(),
        ...
        update_bar_group_y: function () {
            var vp = $('#mcb-bar-vertical_alignment input:checked').val(),
                f = $('#mcb-bar-is_fixed').prop('checked'),
