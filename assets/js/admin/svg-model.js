(function ($, document) {
  ...
  var model = {
    ...
    init: function () {
      ...
      $('#mcb-model-placeholder').attr('height', ($('#mcb-bar-placeholder_height').val() > 0) ? $('#mcb-bar-placeholder_height').val() / 2 : 0)
    ...
    onReady: function () {
      ...
      $('#mcb-model-mobile-draggable').mousedown(function (event) {
        ...
        if (f) {
          bgt = $('#mcb-model-bar-group').attr('transform').match(/\w+\(([^,)]+),([^)]+)\)/)
        ...
        if (model.dragvars.active && (model.min.mobile + model.dragvars.dt <= y && y <= model.max.mobile - model.dragvars.db)) {
          ...
          var bgt = $('#mcb-model-bar-group').attr('transform').match(/\w+\(([^,)]+),([^)]+)\)/)

          var f = $('#mcb-bar-is_fixed').prop('checked')

          if (f) {
            $('#mcb-model-bar-group').attr('transform', 'translate(' + bgt[1] + ',' + (event.clientY - model.dragvars.dby) + ')')
          }
    ...
    update_bar_group_x: function () {
      ...
      var w = $('#mcb-model-bar').attr('width')
    ...
    update_bar_group_y: function () {
      ...
      var sh = ($('#mcb-bar-space_height').val() > 0) ? $('#mcb-bar-space_height').val() / 2 : 0
      ...
      if (f) {
        mgt = $('#mcb-model-mobile-group').attr('transform').match(/\w+\(([^,)]+),([^)]+)\)/)
    ...
    update_mobile_group_y: function () {
      ...
      if (vp === 'top' && mgt[2] < model.min.mobile) {
        mgt[2] = model.min.mobile
      ...
      } else if (vp === 'bottom' && mgt[2] > model.max.mobile) {
        mgt[2] = model.max.mobile
        $('#mcb-model-mobile-group').attr('transform', 'translate(' + mgt[1] + ',' + (mgt[2] + +ph) + ')')
    ...
    update_border_bottom_y: function () {
      ...
      var bbh = $('#mcb-model-border-bottom').attr('height')
    ...
    update_borders_height: function () {
      ...
      var bw = ($('#mcb-bar-border_width').val() > 0) ? $('#mcb-bar-border_width').val() / 2 : 0
