...
final class Input
{
    ...
    public function sanitize( $contacts = [] )
    {
        ...
        foreach ( $contacts as $contact_id => &$contact )
        {
            ...
            if ( $contact['icon'] && ! $this->in_icons( $contact['icon'] ))
            {
                unset( $contacts[$contact_id] );
            }

            // remove contact if invalid 'type'
            if ( ! in_array( $contact['type'], $contact_types ))
            {
                unset( $contacts[$contact_id] );
            }
        ...
        foreach ( $contacts as $contact_id => $contact )
        {
            ...
            if ( isset( $contact['parameters'] ) && is_array( $contact['parameters'] ))
            {
                ...
                foreach ( $contact['parameters'] as $parameter_id => $parameter )
                {
                    ...
                    // santitize 'value'
                    $sanitized_contact['parameters'][$parameter_id]['value'] = Validator::sanitize_parameter_value( $parameter['value'], $parameter_type['field'] );
                }
            }
     ...
     */
    private function in_icons( $classes )
    {
        $classes = explode( ' ', $classes );
        $name    = substr( $classes[1], 3 );
        $icons   = self::icons();
