...
final class Mobile_Contact_Bar_Renderer_V1 {
	...
	public static function wp_head() {
		?>
		<style id="mobile-contact-bar-css" type="text/css" media="screen"><?php echo wp_kses( self::$option['styles'], array( "\'", '\"' ) ); ?></style>
		<?php
	}
	...
	public static function render_html( $contacts, $settings ) {
		if ( 1 === did_action( 'mobile_contact_bar_public_render_html' ) ) {
			...
			foreach ( $contacts as $id => $contact ) {
				...
				$html .= sprintf(
					'<li><a data-rel="external" href="%s"%s><i class="fa %s fa-%s"></i></a></li>',
					$url,
					$new_tab,
