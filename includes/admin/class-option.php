...
final class Mobile_Contact_Bar_Option
{
    ...
    public static function callback_output_setting( $args )
    {
        switch( $args['setting']['type'] )
        {
            ...
            case 'radio':
                printf( '<fieldset class="mcb-radio-label-wrap" id="mcb-%s-%s">',
                    esc_attr( $args['section_id'] ),
                    esc_attr( $args['setting_id'] )
                );
                foreach( $args['setting']['options'] as $value => $label )
            ...
            case 'radio-image':
                printf( '<fieldset class="mcb-radio-image-wrap" id="mcb-%s-%s">',
                    esc_attr( $args['section_id'] ),
                    esc_attr( $args['setting_id'] )
                );
    ...
    private static function output_contact_th( $contact_id, $contact )
    {
        ...
        // 'checkbox' input
        $out .= sprintf( '<li class="mcb-contact-checkbox"><input name="' . $prefix . '[checked]" value="1" %s type="checkbox"></li>',
        ...
        {
            // 'title' input
            $out .= sprintf( '<li class="mcb-contact-title"><input name="' . $prefix . '[title]" value="%s" placeholder="%s" type="text"></li>',
    ...
    private static function output_contact_td( $args )
    {
        ...
        // 'placeholder' hidden
        $out .= sprintf( '<input name="' . MOBILE_CONTACT_BAR__NAME . '[contacts][%s][placeholder]" value="%s" type="hidden">',
        ...
        {
            // 'URI' hidden
            $out .= sprintf( '<input name="' . MOBILE_CONTACT_BAR__NAME . '[contacts][%s][uri]" value="%s" type="hidden">',
                esc_attr( $args['contact_id'] ),
                $sanitized_uri
            );
            // 'URI'
    ...
    public static function callback_sanitize_option( $input )
    {
        ...
        foreach( $settings as $section_id => &$section )
        {
            ...
            $section = array_replace(
                array_map( function( $setting ) { if( 'checkbox' == $setting['type'] ) { return 0; }}, self::$settings[$section_id] ),
                $section
            );
            // all settings will be saved, at least with their default values
            $section = array_replace(
                array_map( function( $setting ) { return $setting['default']; }, self::$settings[$section_id] ),
                $section
            );
        }
        unset( $section );
    ...
    public static function pre_update_option( $new_value, $old_value = array() )
    {
        ...
        $contacts = array_filter( $new_value['contacts'], function( $contact ) { return $contact['checked']; });
    ...
    public static function admin_footer()
    {
        ...
        <script type="text/html" id="mcb-tmpl-parameter">
            <tr class="mcb-parameter">
                ...
                <td>
                    <?php echo self::output_parameter_td( array(
                        'parameter_id' => '',
                        'parameter'    => array(
                            'key'         => '',
                            'type'        => 'text',
    ...
    public static function ajax_add_contact()
    {
        ...
        if( $contact )
        {
            ...
            $out .= '<tr class="mcb-contact">';
                ...
                $out .= '</th>';
                $out .= '<td>';
                    $out .= self::output_contact_td( array( 'contact_id' => '', 'contact' => $contact, ));
                $out .= '</td>';
            $out .= '</tr>';
            ...
            if( isset( $contact['parameters'] ) && $contact['parameters'] )
            {
                ...
                foreach( $contact['parameters'] as $parameter_id => $parameter )
                {
                    ...
                    $out .= '<tr class="mcb-parameter hidden">';
                        ...
                        $out .= '</th>';
                        $out .= '<td>';
                            $out .= self::output_parameter_td( array(
                                'parameter_id' => $parameter_id,
                                'parameter'    => $parameter,
                                'contact_id'   => '',
                                'contact_type' => $contact['type'],
