<?php
/**
 * Mobile Contact Bar Option Page
 *
...
final class Mobile_Contact_Bar_Page {
	...
	public static function on_activation( $network_wide = false ) {
		...
		$readme_data = get_file_data(
			plugin_dir_path( MOBILE_CONTACT_BAR__PATH ) . 'readme.txt',
			array(
		...
		if ( version_compare( PHP_VERSION, $readme_data['Requires PHP'], '<' ) ) {
			...
			wp_die(
				sprintf(
					...
					esc_html__( 'Mobile Contact Bar requires at least PHP version %1$s. You are running version %2$s. Please upgrade and try again.', 'mobile-contact-bar' ),
					esc_attr( $readme_data['Requires PHP'] ),
			...
			wp_die(
				sprintf(
					...
					esc_html__( 'Mobile Contact Bar requires at least WordPress version %1$s. You are running version %2$s. Please upgrade and try again.', 'mobile-contact-bar' ),
					esc_attr( $readme_data['Requires at least'] ),
					esc_attr( $wp_version )
				),
				'Plugin Activation Error',
				array( 'back_link' => true )
	...
	public static function render_help_tab_sms() {
		...
		<ul class="mcb-query-parameters">
			<li>
				...
				<span><?php esc_html_e( 'Text to appear in the body of the message (it does not always work).', 'mobile-contact-bar' ); ?></span>
			</li>
		</ul>
	...
	public static function render_help_tab_mailto() {
		...
		<ul class="mcb-query-parameters">
			<li>
				...
				<span><?php esc_html_e( 'Text to appear in the subject line of the message.', 'mobile-contact-bar' ); ?></span>
			</li>
			<li>
				<span class="mcb-query-parameter-key">body</span>
				<span><?php esc_html_e( 'Text to appear in the body of the message.', 'mobile-contact-bar' ); ?></span>
			</li>
			<li>
				<span class="mcb-query-parameter-key">cc</span>
				<span><?php esc_html_e( 'Addresses to be included in the carbon copy section of the message. Separate addresses with commas.', 'mobile-contact-bar' ); ?></span>
			</li>
			<li>
				<span class="mcb-query-parameter-key">bcc</span>
				<span><?php esc_html_e( 'Addresses to be included in the blind carbon copy section of the message. Separate addresses with commas.', 'mobile-contact-bar' ); ?></span>
			</li>
		</ul>
		<p class="mcb-tab-status-green"><?php esc_html_e( 'Standardised protocol', 'mobile-contact-bar' ); ?></p>
		<?php
	...
	public static function default_option() {
		...
		$option['contacts'] = Mobile_Contact_Bar_Contact_Sample::admin_get_contact();
		$option             = Mobile_Contact_Bar_Option::pre_update_option( $option ); /* styles */
